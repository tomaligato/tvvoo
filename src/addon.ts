...
// Preferred brand order for the "sport" dossier: BeIN Sport, Eurosport, RMC Sport, Dazn, then everything else A-Z.
// Matching is prefix-based (accent/case/punctuation-insensitive) so "BeIN SPORTS 1", "Eurosport 2", "RMC Sport News",
// "DAZN La Liga", etc. all resolve to their brand regardless of numbering/suffix.
const SPORT_CHANNEL_ORDER: string[] = [
    'bein sport', 'eurosport', 'rmc sport', 'dazn'
];
function sportChannelRank(baseName: string): number {
    const n = (baseName || '')
        .normalize('NFD').replace(/[\u0300-\u036f]/g, '')
        .toLowerCase()
        .replace(/[^a-z0-9]+/g, ' ')
        .trim();
    for (let i = 0; i < SPORT_CHANNEL_ORDER.length; i++) {
        const key = SPORT_CHANNEL_ORDER[i];
        if (n === key || n.startsWith(key + ' ') || n.startsWith(key)) return i;
    }
    return Number.MAX_SAFE_INTEGER;
}
...
async function getOrFetchCountryCatalog(cid: string): Promise<any[]> {
    try {
        ...
        // IMPORTANT: assign the in-flight promise synchronously (no `await` before this line) so that
        // any other concurrent call for the same `cid` that reaches the hasOwnProperty check above
        // right after this one is guaranteed to see the slot already reserved, instead of racing to
        // start its own duplicate network fetch (which previously caused several parallel fetches for
        // the same country whenever multiple dossier/catalog requests arrived at once, e.g. right
        // after a cold start).
        inflightCatalogFetch[cid] = (async () => {
            const sig = await getVavooSignature(null);
            if (!sig) return [];
            return await withCatalogFetchSlot(async () => {
...
builder.defineCatalogHandler(async ({ id, type, extra }: { id: string; type: string; extra?: any }) => {
    try {
        ...
        if (id.startsWith('vavoo_cat_')) {
            ...
            } else if (bucketId === 'sport') {
                catRows.sort((a, b) => {
                    const ra = sportChannelRank(a.baseName), rb = sportChannelRank(b.baseName);
                    if (ra !== rb) return ra - rb;
                    return a.baseName.localeCompare(b.baseName, 'fr', { sensitivity: 'base' });
                });
            } else {
            ...
            // Group by canonical channel identity (ignoring country + quality/source-tag suffixes like
            // HD/FHD/UHD/4K, resolutions (720p/1080p/...), and source labels (Backup/Feed/Mirror/...))
            // so the same channel appearing from several countries/sources/mirrors shows only once.
            // Handles both space-separated tags ("BeIN Sports 1 Backup") and fused ones ("720pFeed").
            const SOURCE_TAG_RE = /\b(backup|feed|mirror|alt(?:ernate)?|source|stream|sub|vip|test|copy|bak|fallback)\w*\b/g;
            const SOURCE_TAG_TEST_RE = /\b(backup|feed|mirror|alt(?:ernate)?|source|stream|sub|vip|test|copy|bak|fallback)\w*\b/i;
            function canonicalChannelKey(name: string): string {
                return name
                    .normalize('NFD').replace(/[\u0300-\u036f]/g, '')
                    .toLowerCase()
                    // strip resolution tags first (also handles "720pfeed" -> "feed", by removing "720p"
                    // in place so the following word becomes its own token once spaces are collapsed)
                    .replace(/\d{3,4}p/g, ' ')
                    .replace(/\b(fhd|uhd|hd|sd|4k|hevc|h265|h264|x264|x265)\b/g, ' ')
                    .replace(SOURCE_TAG_RE, ' ')
                    .replace(/[^a-z0-9]+/g, ' ')
                    .trim();
            }
            const hasQualitySuffix = (name: string) => /\b(fhd|uhd|hd|sd|4k)\b/i.test(name) || /\d{3,4}p/i.test(name) || SOURCE_TAG_TEST_RE.test(name);
            ...
            for (const r of catRows) {
                ...
                if (!existing) {
                    ...
                    // Prefer the cleaner name (without HD/FHD/UHD/4K/resolution/source tags) as the representative
                    catGroups.set(key, { baseName: r.baseName, countryId: r.countryId });
                }
            }
            ...
            if (!catSearchQ && catMetas.length > 0) catMetasCache[catCacheKey] = { updatedAt: Date.now(), metas: catMetas };
