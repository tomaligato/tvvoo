    ...
    app.use((req: Request, res: Response, next: NextFunction) => {
        try {
            ...
            if (providedFromPath === VAVOO_ACCESS_TOKEN || providedFromQuery === VAVOO_ACCESS_TOKEN) {
                ...
                // === LA CORRECTION : NETTOYAGE DE L'URL ===
                // 1. On retire le "-tok_XXXX" ou "_tok_XXXX" du chemin pour le routeur et le cache
                if (m) {
                    req.url = req.url.replace(/[-_]?tok_[A-Za-z0-9_-]+/, '');
                }
                // 2. On nettoie le paramètre query pour éviter de polluer les clés de cache internes
                if (req.query.token) {
